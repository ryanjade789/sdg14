...
if (isset($_POST['submit'])) {
 ...
 // $total_number = $_POST['total_number'];
  ...
  $sql = "INSERT INTO `14.4.1` (`yes`,`title`, `description`, `cost`, `source`) VALUES ('$yes','$title','$description', '$cost', '$source')";
...
<title>SDG 14.4.1 | Water discharge guidelines and standards</title>
...
<div class="main" style="background-color:#C31F33;">
  ...
  <div class="input-container">
    ...
    $query = "SELECT Count(*) AS total  FROM `14.4.1`"; // SQL query to fetch all table data
    ...
    while ($row = mysqli_fetch_assoc($result)) {
     ...
     $totalno= $total;
     $yes = 0;
     $query = "SELECT * FROM `14.4.1`"; // SQL query to fetch all table data
    $result = mysqli_query($conn, $query); // sending the query to the database
    while ($row = mysqli_fetch_assoc($result)) {
      if ($row['yes'] == 'yes'){
        $yes++;
...
<!--============================================================================================= 
 ...
 ============================================================================================= -->
  ...
  <div class="content">
    <br>
    <h3>14.4.1 Water discharge guidelines and standards</h3>
    <p>Have water quality standards and guidelines for water discharges (to uphold water quality in order to protect ecosystems, wildlife, and
...
<div class="mb-2 mt-2"><i class="fa fa-search"></i>
      <label>Does your campus have water quality standards and guidelines for water discharges?</label><br>
...
<div class="table-container">
  ...
  <!--    <th scope="col" style="width: 30px;">#</th>
      ...
      <th scope="col" style="width: 150px;">Standards and Guidelines</th>
    ...
    <?php
            include "includes/config.php";   // CHANGE THIS WITH YOUR ACTUAL CONNECTION TO DATABASE ex: conn.php
            $query = "SELECT * FROM `14.4.1` ORDER BY `total` DESC"; // SQL query to fetch all table data
            $result = mysqli_query($conn, $query); // sending the query to the database
              ...
              $id = $row['total'];
               ...
               // echo "<td>{$total_number}</td>";
                ...
                echo "<td class ='text-center' style='width:100px'>
                          <a href='edit/edit_14.4.1.php?update&research_id={$id}' class='btn btn-primary'>
                              <i class='fa fa-edit'></i> 
                          </a>
                          <a href='delete/delete_14.4.1.php?delete={$id}' class='btn btn-danger'>
